...
@interface TFY_AddressPickerView ()<UIPickerViewDataSource, UIPickerViewDelegate>
// 地址选择器
...
// 选中的省
@property(nonatomic, strong) TFY_ProvinceModel *selectProvinceModel;
// 选中的市
@property(nonatomic, strong) TFY_CityModel *selectCityModel;
// 选中的区
@property(nonatomic, strong) TFY_AreaModel *selectAreaModel;
// 记录省选中的位置
...
+ (void)showAddressPickerWithSelectIndexs:(NSArray <NSNumber *>*)selectIndexs
                              resultBlock:(TFYAddressResultBlock)resultBlock {
    [self showAddressPickerWithMode:TFYAddressPickerModeArea dataSource:nil selectIndexs:selectIndexs isAutoSelect:NO resultBlock:resultBlock];
}
...
+ (void)showAddressPickerWithMode:(TFYAddressPickerMode)mode
                     ...
                     isAutoSelect:(BOOL)isAutoSelect
                      resultBlock:(TFYAddressResultBlock)resultBlock {
    // 创建地址选择器
    TFY_AddressPickerView *addressPickerView = [[TFY_AddressPickerView alloc] initWithPickerMode:mode];
...
- (void)handlerPickerData {
    if (self.dataSourceArr && self.dataSourceArr.count > 0) {
        ...
        // 如果传的值是解析好的模型数组
        if ([item isKindOfClass:[TFY_ProvinceModel class]]) {
        ...
        NSArray *dataSource = [self picker_addressJsonArray];
...
- (NSArray *)picker_addressJsonArray {
    static NSArray *cityArray = nil;
    if (!cityArray) {
        // 获取 TFY_AddressPickerView.bundle
        NSBundle *containnerBundle = [NSBundle bundleForClass:[TFY_AddressPickerView class]];
        NSString *bundlePath = [containnerBundle pathForResource:@"TFY_CityBundle" ofType:@"bundle"];
        NSBundle *addressPickerBundle = [NSBundle bundleWithPath:bundlePath];
        
        // 获取bundle中的JSON文件
        NSString *filePath = [addressPickerBundle pathForResource:@"TFY_City" ofType:@"json"];
...
#pragma mark - 获取模型数组
- (NSArray <TFY_ProvinceModel *>*)getProvinceModelArr:(NSArray *)dataSourceArr {
    NSMutableArray *tempArr1 = [NSMutableArray array];
    for (NSDictionary *proviceDic in dataSourceArr) {
        TFY_ProvinceModel *proviceModel = [[TFY_ProvinceModel alloc]init];
        ...
        for (NSDictionary *cityDic in cityList) {
            TFY_CityModel *cityModel = [[TFY_CityModel alloc]init];
            ...
            for (NSDictionary *areaDic in areaList) {
                TFY_AreaModel *areaModel = [[TFY_AreaModel alloc]init];
...
- (void)handlerDefaultSelectValue {
    ...
    if (self.pickerMode == TFYAddressPickerModeProvince || self.pickerMode == TFYAddressPickerModeCity || self.pickerMode == TFYAddressPickerModeArea) {
        if (self.selectIndexs.count > 0) {
            ...
            [self.provinceModelArr enumerateObjectsUsingBlock:^(TFY_ProvinceModel *  _Nonnull model, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    if (self.pickerMode == TFYAddressPickerModeCity || self.pickerMode == TFYAddressPickerModeArea) {
        ...
        if (self.selectIndexs.count > 0) {
            ...
            [self.cityModelArr enumerateObjectsUsingBlock:^(TFY_CityModel *  _Nonnull model, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    if (self.pickerMode == TFYAddressPickerModeArea) {
        ...
        if (self.selectIndexs.count > 0) {
            ...
            [self.areaModelArr enumerateObjectsUsingBlock:^(TFY_AreaModel *  _Nonnull model, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (NSArray *)getCityModelArray:(NSInteger)provinceIndex {
    TFY_ProvinceModel *provinceModel = self.provinceModelArr[provinceIndex];
    // 返回城市模型数组
...
- (NSArray *)getAreaModelArray:(NSInteger)provinceIndex cityIndex:(NSInteger)cityIndex {
    ...
    if (provinceModel.citylist && provinceModel.citylist.count > 0) {
        TFY_CityModel *cityModel = provinceModel.citylist[cityIndex];
        // 返回地区模型数组
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView {
    switch (self.pickerMode) {
        case TFYAddressPickerModeProvince:
            return 1;
            break;
        case TFYAddressPickerModeCity:
            return 2;
            break;
        case TFYAddressPickerModeArea:
            return 3;
            break;
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view {
    ...
    if (component == 0) {
        TFY_ProvinceModel *model = self.provinceModelArr[row];
        label.text = model.name;
    } else if (component == 1) {
        TFY_CityModel *model = self.cityModelArr[row];
        label.text = model.name;
    } else if (component == 2) {
        TFY_AreaModel *model = self.areaModelArr[row];
        label.text = model.name;
    }
...
- (void)reloadData {
    ...
    // 3.滚动到选择的地区
    if (self.pickerMode == TFYAddressPickerModeProvince) {
        [self.pickerView selectRow:self.provinceIndex inComponent:0 animated:YES];
    } else if (self.pickerMode == TFYAddressPickerModeCity) {
        [self.pickerView selectRow:self.provinceIndex inComponent:0 animated:YES];
        [self.pickerView selectRow:self.cityIndex inComponent:1 animated:YES];
    } else if (self.pickerMode == TFYAddressPickerModeArea) {
        [self.pickerView selectRow:self.provinceIndex inComponent:0 animated:YES];
        [self.pickerView selectRow:self.cityIndex inComponent:1 animated:YES];
...
- (TFY_ProvinceModel *)selectProvinceModel {
    if (!_selectProvinceModel) {
        _selectProvinceModel = [[TFY_ProvinceModel alloc]init];
    }
    return _selectProvinceModel;
}

- (TFY_CityModel *)selectCityModel {
    if (!_selectCityModel) {
        _selectCityModel = [[TFY_CityModel alloc]init];
...
- (TFY_AreaModel *)selectAreaModel {
    if (!_selectAreaModel) {
        _selectAreaModel = [[TFY_AreaModel alloc]init];
