...
/// 日期选择器的类型
typedef NS_ENUM(NSInteger, TFYDatePickerStyle) {
    TFYDatePickerStyleSystem,   // 系统样式：使用 UIDatePicker
    TFYDatePickerStyleCustom    // 自定义样式：使用 UIPickerView
};
...
/** 日期选择器的类型 */
@property (nonatomic, assign) TFYDatePickerStyle style;
/** 日期的格式 */
...
+ (void)showDatePickerWithMode:(TFYDatePickerMode)mode
                   ...
                   resultBlock:(TFYDateResultBlock)resultBlock {
...
+ (void)showDatePickerWithMode:(TFYDatePickerMode)mode
                  ...
                  isAutoSelect:(BOOL)isAutoSelect
                   resultBlock:(TFYDateResultBlock)resultBlock
                   resultRangeBlock:(TFYDateResultRangeBlock)resultRangeBlock {
    // 创建日期选择器
    TFY_DatePickerView *datePickerView = [[TFY_DatePickerView alloc]init];
...
- (void)handlerPickerData {
    ...
    BOOL minMoreThanMax = [self picker_compareDate:self.minDate targetDate:self.maxDate dateFormat:self.dateFormatter] == NSOrderedDescending;
    ...
    // 4.设置选择器日期数据
    if (self.style == TFYDatePickerStyleCustom) {
        [self setupDateArray];
    }
    ...
    if (self.selectValue && ([self.selectValue isEqualToString:self.lastRowContent] || [self.selectValue isEqualToString:self.firstRowContent])) {
        ...
        if (self.pickerMode == TFYDatePickerModeYMDH && self.isShowAMAndPM) {
            self.hourIndex = (self.mSelectDate.picker_hour < 12 ? 0 : 1);
            self.mSelectValue = [NSString stringWithFormat:@"%04d-%02d-%02d %@", (int)self.mSelectDate.picker_year, (int)self.mSelectDate.picker_month, (int)self.mSelectDate.picker_day, [self getHourString]];
        } else {
            self.mSelectValue = [self picker_stringFromDate:self.mSelectDate dateFormat:self.dateFormatter];
        }
    }
}
...
- (void)setupDateArray {
    if (self.selectValue && ([self.selectValue isEqualToString:self.lastRowContent] || [self.selectValue isEqualToString:self.firstRowContent])) {
        switch (self.pickerMode) {
            ...
            {
                self.yearArr = [self getYearArr];
                self.monthArr = [self getMonthArr:self.mSelectDate.picker_year];
                self.dayArr = nil;
                self.hourArr = nil;
                self.minuteArr = nil;
                self.secondArr = nil;
            ...
            case TFYDatePickerModeHM:
            {
                self.yearArr = [self getYearArr];
                self.monthArr = [self getMonthArr:self.mSelectDate.picker_year];
                self.dayArr = [self getDayArr:self.mSelectDate.picker_year month:self.mSelectDate.picker_month];
                self.hourArr = [self getHourArr:self.mSelectDate.picker_year month:self.mSelectDate.picker_month day:self.mSelectDate.picker_day];
                self.minuteArr = nil;
                self.secondArr = nil;
                self.monthWeekArr = nil;
                self.yearWeekArr = nil;
            ...
            case TFYDatePickerModeMS:
            {
                ...
                self.dayArr = [self getDayArr:self.mSelectDate.picker_year month:self.mSelectDate.picker_month];
                self.hourArr = [self getHourArr:self.mSelectDate.picker_year month:self.mSelectDate.picker_month day:self.mSelectDate.picker_day];
                self.minuteArr = [self getMinuteArr:self.mSelectDate.picker_year month:self.mSelectDate.picker_month day:self.mSelectDate.picker_day hour:self.mSelectDate.picker_hour];
                self.secondArr = nil;
                self.monthWeekArr = nil;
                self.yearWeekArr = nil;
                self.quarterArr = nil;
                ...
                break;
            case TFYDatePickerModeYMW:
            {
                self.yearArr = [self getYearArr];
                self.monthArr = [self getMonthArr:self.mSelectDate.picker_year];
                self.monthWeekArr = [self getMonthWeekArr:self.mSelectDate.picker_year month:self.mSelectDate.picker_month];
                self.yearWeekArr = nil;
                self.quarterArr = nil;
                self.dayArr = nil;
                self.hourArr = nil;
                ...
                break;
            case TFYDatePickerModeYW:
            {
                self.yearArr = [self getYearArr];
                self.monthArr = nil;
                self.monthWeekArr = nil;
                self.yearWeekArr = [self getYearWeekArr:self.mSelectDate.picker_year];
                self.quarterArr = nil;
                self.dayArr = nil;
                self.hourArr = nil;
                self.minuteArr = nil;
                ...
                break;
            case TFYDatePickerModeYQ:
            {
                self.yearArr = [self getYearArr];
                self.monthArr = nil;
                self.monthWeekArr = nil;
                self.yearWeekArr = nil;
                self.quarterArr = [self getQuarterArr:self.mSelectDate.picker_year];;
                self.dayArr = nil;
                self.hourArr = nil;
                self.minuteArr = nil;
                self.secondArr = nil;
        ...
        self.hourArr = [self getHourArr:self.mSelectDate.picker_year month:self.mSelectDate.picker_month day:self.mSelectDate.picker_day];
        self.minuteArr = [self getMinuteArr:self.mSelectDate.picker_year month:self.mSelectDate.picker_month day:self.mSelectDate.picker_day hour:self.mSelectDate.picker_hour];
        self.secondArr = [self getSecondArr:self.mSelectDate.picker_year month:self.mSelectDate.picker_month day:self.mSelectDate.picker_day hour:self.mSelectDate.picker_hour minute:self.mSelectDate.picker_minute];
...
- (void)setupDateFormatter:(TFYDatePickerMode)mode {
    switch (mode) {
        case TFYDatePickerModeDate:
        {
            self.dateFormatter = @"yyyy-MM-dd";
            self.style = TFYDatePickerStyleSystem;
            _datePickerMode = UIDatePickerModeDate;
        }
            break;
        case TFYDatePickerModeDateAndTime:
        {
        ...
        case TFYDatePickerModeTime:
        {
            ...
            break;
        case TFYDatePickerModeCountDownTimer:
        {
            self.dateFormatter = @"HH:mm";
            self.style = TFYDatePickerStyleSystem;
        ...
        case TFYDatePickerModeYMDHMS:
        {
            ...
            self.style = TFYDatePickerStyleCustom;
        ...
        case TFYDatePickerModeYMDH:
        {
            ...
            self.style = TFYDatePickerStyleCustom;
            self.unitArr = @[[self getYearUnit], [self getMonthUnit], [self getDayUnit], self.pickerMode == TFYDatePickerModeYMDH && self.isShowAMAndPM ? @"" : [self getHourUnit]];
        }
            break;
        case TFYDatePickerModeMDHM:
        {
...
- (void)reloadDateArrayWithUpdateMonth:(BOOL)updateMonth updateDay:(BOOL)updateDay updateHour:(BOOL)updateHour updateMinute:(BOOL)updateMinute updateSecond:(BOOL)updateSecond
    ...
    if (updateMonth) {
        NSString *lastSelectMonth = [self getMDHMSNumber:self.mSelectDate.picker_month];
    ...
    // 1/1.更新 yearWeekArr
    if (updateWeekOfYear) {
        NSString *lastSelectWeekOfYear = [self getMDHMSNumber:self.mSelectDate.picker_yearWeek];
    ...
    // 1/1.更新 quarterArr
    if (updateQuarter) {
        NSString *lastSelectQuarter = [self getMDHMSNumber:self.mSelectDate.picker_quarter];
    ...
    if (updateDay) {
        NSString *lastSelectDay = [self getMDHMSNumber:self.mSelectDate.picker_day];
    ...
    if (updateWeekOfMonth) {
        NSString *lastWeekOfMonth = [self getMDHMSNumber:self.mSelectDate.picker_monthWeek];
    ...
    if (updateHour) {
        NSString *lastSelectHour = [self getMDHMSNumber:self.mSelectDate.picker_hour];
    ...
    if (updateMinute) {
        NSString *lastSelectMinute = [self getMDHMSNumber:self.mSelectDate.picker_minute];
    ...
    if (updateSecond) {
        NSString *lastSelectSecond = [self getMDHMSNumber:self.mSelectDate.picker_second];
...
- (void)scrollToSelectDate:(NSDate *)selectDate animated:(BOOL)animated {
    self.yearIndex = [self getIndexWithArray:self.yearArr object:[self getYearNumber:selectDate.picker_year]];
    self.monthIndex = [self getIndexWithArray:self.monthArr object:[self getMDHMSNumber:selectDate.picker_month]];
    self.dayIndex = [self getIndexWithArray:self.dayArr object:[self getMDHMSNumber:selectDate.picker_day]];
    if (self.pickerMode == TFYDatePickerModeYMDH && self.isShowAMAndPM) {
        self.hourIndex = selectDate.picker_hour < 12 ? 0 : 1;
    } else {
        self.hourIndex = [self getIndexWithArray:self.hourArr object:[self getMDHMSNumber:selectDate.picker_hour]];
    }
    self.minuteIndex = [self getIndexWithArray:self.minuteArr object:[self getMDHMSNumber:selectDate.picker_minute]];
    self.secondIndex = [self getIndexWithArray:self.secondArr object:[self getMDHMSNumber:selectDate.picker_second]];
    ...
    } else if (self.pickerMode == TFYDatePickerModeYM) {
        if ([self.pickerStyle.language hasPrefix:@"zh"]) {
            indexArr = @[@(self.yearIndex), @(self.monthIndex)];
        } else {
            indexArr = @[@(self.monthIndex), @(self.yearIndex)];
        }
    } else if (self.pickerMode == TFYDatePickerModeY) {
    ...
    } else if (self.pickerMode == TFYDatePickerModeHM) {
    ...
    } else if (self.pickerMode == TFYDatePickerModeYMW) {
        indexArr = @[@(self.yearIndex), @(self.monthIndex), @(self.monthWeekIndex)];
    } else if (self.pickerMode == TFYDatePickerModeYW) {
        indexArr = @[@(self.yearIndex), @(self.yearWeekIndex)];
    } else if (self.pickerMode == TFYDatePickerModeYQ) {
...
- (void)scrollToCustomString:(BOOL)animated {
    switch (self.pickerMode) {
        ...
        case TFYDatePickerModeYQ:
        {
            ...
            if ((self.pickerMode == TFYDatePickerModeYMD || self.pickerMode == TFYDatePickerModeYMW) && ![self.pickerStyle.language hasPrefix:@"zh"]) {
                component = 2;
            } else if ((self.pickerMode == TFYDatePickerModeYM || self.pickerMode == TFYDatePickerModeYQ) && ![self.pickerStyle.language hasPrefix:@"zh"]) {
                component = 1;
            }
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    if (self.pickerMode == TFYDatePickerModeYMDHMS) {
        ...
        if (![yearString isEqualToString:self.lastRowContent] && ![yearString isEqualToString:self.firstRowContent]) {
            ...
            self.mSelectDate = [NSDate picker_setYear:year month:month day:day hour:hour minute:minute second:second];
        ...
        if (![yearString isEqualToString:self.lastRowContent] && ![yearString isEqualToString:self.firstRowContent]) {
            ...
            int hour = [[self getHourString] intValue];
            int minute = [[self getMinuteString] intValue];
            self.mSelectDate = [NSDate picker_setYear:year month:month day:day hour:hour minute:minute];
        ...
        if (![yearString isEqualToString:self.lastRowContent] && ![yearString isEqualToString:self.firstRowContent]) {
            ...
            self.mSelectDate = [NSDate picker_setYear:year month:month day:day hour:hour];
        } else {
            self.mSelectDate = self.addToNow ? [NSDate date] : nil;
            if ([yearString isEqualToString:self.lastRowContent]) {
                self.mSelectValue = self.lastRowContent;
        ...
        if (![yearString isEqualToString:self.lastRowContent] && ![yearString isEqualToString:self.firstRowContent]) {
            ...
            int month = [[self getMonthString] intValue];
            int day = [[self getDayString] intValue];
            self.mSelectDate = [NSDate picker_setYear:year month:month day:day];
        ...
        if (![yearString isEqualToString:self.lastRowContent] && ![yearString isEqualToString:self.firstRowContent]) {
            ...
            int year = [[self getYearString] intValue];
            int month = [[self getMonthString] intValue];
            self.mSelectDate = [NSDate picker_setYear:year month:month];
        ...
        if (![yearString isEqualToString:self.lastRowContent] && ![yearString isEqualToString:self.firstRowContent]) {
            ...
            int year = [[self getYearString] intValue];
            self.mSelectDate = [NSDate picker_setYear:year];
        ...
        if (![hourString isEqualToString:self.lastRowContent] && ![hourString isEqualToString:self.firstRowContent]) {
            ...
            int minute = [[self getMinuteString] intValue];
            int second = [[self getSecondString] intValue];
            self.mSelectDate = [NSDate picker_setHour:hour minute:minute second:second];
        ...
        if (![hourString isEqualToString:self.lastRowContent] && ![hourString isEqualToString:self.firstRowContent]) {
            ...
            int hour = [[self getHourString] intValue];
            int minute = [[self getMinuteString] intValue];
            self.mSelectDate = [NSDate picker_setHour:hour minute:minute];
        ...
        if (![minuteString isEqualToString:self.lastRowContent] && ![minuteString isEqualToString:self.firstRowContent]) {
            ...
            int minute = [[self getMinuteString] intValue];
            int second = [[self getSecondString] intValue];
            self.mSelectDate = [NSDate picker_setMinute:minute second:second];
        ...
        if (![yearString isEqualToString:self.lastRowContent] && ![yearString isEqualToString:self.firstRowContent]) {
            ...
            self.mSelectDate = [NSDate picker_setYear:year month:month weekOfMonth:week];
        ...
        if (![yearString isEqualToString:self.lastRowContent] && ![yearString isEqualToString:self.firstRowContent]) {
            ...
            self.mSelectDate = [NSDate picker_setYear:year weekOfYear:week];
        ...
        if (![yearString isEqualToString:self.lastRowContent] && ![yearString isEqualToString:self.firstRowContent]) {
            ...
            self.mSelectDate = [NSDate picker_setYear:year quarter:quarter];
    ...
    if (self.nonSelectableDates && self.nonSelectableDates.count > 0 && ![self.mSelectValue isEqualToString:self.lastRowContent] && ![self.mSelectValue isEqualToString:self.firstRowContent]) {
        for (NSDate *date in self.nonSelectableDates) {
            if ([self picker_compareDate:date targetDate:self.mSelectDate dateFormat:self.dateFormatter] == NSOrderedSame) {
...
- (void)didSelectValueChanged:(UIDatePicker *)sender {
    ...
    if (_datePickerMode != UIDatePickerModeCountDownTimer) {
        BOOL selectLessThanMin = [self picker_compareDate:self.mSelectDate targetDate:self.minDate dateFormat:self.dateFormatter] == NSOrderedAscending;
        BOOL selectMoreThanMax = [self picker_compareDate:self.mSelectDate targetDate:self.maxDate dateFormat:self.dateFormatter] == NSOrderedDescending;
...
- (void)reloadData {
    ...
    if (self.style == TFYDatePickerStyleSystem) {
        // 2.刷新选择器（重新设置相关值）
    ...
    } else if (self.style == TFYDatePickerStyleCustom) {
        // 2.刷新选择器
...
- (void)addPickerToView:(UIView *)view {
    ...
    if (self.style == TFYDatePickerStyleSystem) {
        ...
        if (self.showUnitType == TFYShowUnitTypeOnlyCenter) {
            // 添加日期单位到选择器
            [self addUnitLabel];
        }
    }
...
- (void)setPickerMode:(TFYDatePickerMode)pickerMode {
    _pickerMode = pickerMode;
    // 非空，表示二次设置
    if (_datePicker || _pickerView) {
        TFYDatePickerStyle lastStyle = self.style;
        [self setupDateFormatter:pickerMode];
        // 系统样式 切换到 自定义样式
        if (lastStyle == TFYDatePickerStyleSystem && self.style == TFYDatePickerStyleCustom) {
            [self.datePicker removeFromSuperview];
            [self setupPickerView:self.pickerView toView:_containerView];
        }
        // 自定义样式 切换到 系统样式
        if (lastStyle == TFYDatePickerStyleCustom && self.style == TFYDatePickerStyleSystem) {
            [self.pickerView removeFromSuperview];
            [self setupPickerView:self.datePicker toView:_containerView];
        }
        // 刷新选择器数据
        [self reloadData];
        if (self.style == TFYDatePickerStyleCustom && self.showUnitType == TFYShowUnitTypeOnlyCenter) {
            // 添加日期单位到选择器
            [self addUnitLabel];
        }
    }
...
- (void)setAddToNow:(BOOL)addToNow {
    ...
    if (addToNow) {
        ...
        _lastRowContent = [NSBundle picker_localizedStringForKey:@"至今" language:self.pickerStyle.language];
    }
}
...
- (NSArray<NSDate *> *)getSelectRangeDate {
    ...
    switch (self.pickerMode) {
        ...
        case TFYDatePickerModeTime:
        {
            NSDate *tempDate = [self picker_dateFromString:self.mSelectValue dateFormat:self.dateFormatter];
            startDate = tempDate;
        ...
        case TFYDatePickerModeDate:
        {
            NSDate *tempDate = [self picker_dateFromString:self.mSelectValue dateFormat:self.dateFormatter];
            startDate = tempDate;
            endDate = [[tempDate picker_getNewDateToDays:1] dateByAddingTimeInterval:-1];
        }
            break;
        case TFYDatePickerModeYM:
        {
            NSDate *tempDate = [self picker_dateFromString:self.mSelectValue dateFormat:self.dateFormatter];
            startDate = tempDate;
            endDate = [[tempDate picker_getNewDateToMonths:1] dateByAddingTimeInterval:-1];
        }
            break;
        case TFYDatePickerModeY:
        {
            NSDate *tempDate = [self picker_dateFromString:self.mSelectValue dateFormat:self.dateFormatter];
            startDate = tempDate;
            endDate = [[tempDate picker_getNewDateToMonths:12] dateByAddingTimeInterval:-1];
        }
            break;
        case TFYDatePickerModeYMW:
        case TFYDatePickerModeYW:
        {
            ...
            startDate = [tempDate picker_getNewDateToDays:-7];
        }
            break;
        case TFYDatePickerModeYQ:
        {
            startDate = [self picker_dateFromString:self.mSelectValue dateFormat:self.dateFormatter];
            endDate = [[startDate picker_getNewDateToMonths:3] dateByAddingTimeInterval:-1];
        }
            break;
