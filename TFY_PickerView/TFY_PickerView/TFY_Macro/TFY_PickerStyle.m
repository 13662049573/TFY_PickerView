...
#import "TFY_PickerStyle.h"
#import "NSBundle+PickerView.h"

// 标题颜色
#define kTFYDefaultTextColor TFY_RGB_HEX(0x333333, 1.0f)

@implementation TFY_PickerStyle
...
- (UIColor *)maskColor {
    if (!_maskColor) {
        _maskColor = [self picker_colorWithLightColor:TFY_RGB_HEX(0x000000, 0.3f) darkColor:TFY_RGB_HEX(0x666666, 0.3f)];
    }
    return _maskColor;
}
...
- (UIColor *)shadowLineColor {
    if (!_shadowLineColor) {
        if (@available(iOS 13.0, *)) {
            ...
            _shadowLineColor = TFY_RGB_HEX(0xc6c6c8, 1.0f);
        }
    }
...
- (CGFloat)paddingBottom {
    if (_paddingBottom <= 0) {
        _paddingBottom = TFY_BOTTOM_MARGIN;
    }
...
- (UIColor *)titleLineColor {
    if (!_titleLineColor) {
        _titleLineColor = [self picker_colorWithLightColor:TFY_RGB_HEX(0xededee, 1.0f) darkColor:TFY_RGB_HEX(0x18181c, 1.0f)];
    }
...
- (UIColor *)cancelTextColor {
    if (!_cancelTextColor) {
        if (@available(iOS 13.0, *)) {
            ...
            _cancelTextColor = kTFYDefaultTextColor;
        }
    }
...
- (NSString *)cancelBtnTitle {
    if (!_cancelBtnTitle && !_cancelBtnImage) {
        _cancelBtnTitle = [NSBundle picker_localizedStringForKey:@"取消" language:self.language];
    }
...
- (UIColor *)titleTextColor {
    if (!_titleTextColor) {
        if (@available(iOS 13.0, *)) {
            ...
            _titleTextColor = TFY_RGB_HEX(0x999999, 1.0f);
        }
    }
...
- (CGRect)titleLabelFrame {
    if (CGRectEqualToRect(_titleLabelFrame, CGRectZero) || _titleLabelFrame.size.height == 0) {
        _titleLabelFrame = CGRectMake(5 + 60 + 2, 0, TFYGetKeyWindow().bounds.size.width - 2 * (5 + 60 + 2), 44);
    }
...
- (UIColor *)doneTextColor {
    if (!_doneTextColor) {
        if (@available(iOS 13.0, *)) {
            ...
            _doneTextColor = kTFYDefaultTextColor;
        }
    }
...
- (NSString *)doneBtnTitle {
    if (!_doneBtnTitle && !_doneBtnImage) {
        _doneBtnTitle = [NSBundle picker_localizedStringForKey:@"确定" language:self.language];
    }
...
- (CGRect)doneBtnFrame {
    if (CGRectEqualToRect(_doneBtnFrame, CGRectZero) || _doneBtnFrame.size.height == 0) {
        _doneBtnFrame = CGRectMake(TFYGetKeyWindow().bounds.size.width - 60 - 5, 8, 60, 28);
    }
...
- (UIColor *)separatorColor {
    if (!_separatorColor) {
        if (@available(iOS 13.0, *)) {
            ...
            _separatorColor = TFY_RGB_HEX(0xc6c6c8, 1.0f);
        }
    }
...
- (UIColor *)pickerTextColor {
    if (!_pickerTextColor) {
        if (@available(iOS 13.0, *)) {
            ...
            _pickerTextColor = kTFYDefaultTextColor;
        }
    }
...
- (UIColor *)dateUnitTextColor {
    if (!_dateUnitTextColor) {
        if (@available(iOS 13.0, *)) {
            ...
            _dateUnitTextColor = kTFYDefaultTextColor;
        }
    }
...
- (UIColor *)picker_colorWithLightColor:(UIColor *)lightColor darkColor:(UIColor *)darkColor {
    if (@available(iOS 13.0, *)) {
...
+ (instancetype)pickerStyleWithThemeColor:(UIColor *)themeColor {
    TFY_PickerStyle *customStyle = [[self alloc]init];
    if (themeColor) {
        customStyle.cancelTextColor = themeColor;
        customStyle.cancelBorderStyle = TFYBorderStyleSolid;
        customStyle.doneColor = themeColor;
        customStyle.doneTextColor = [UIColor whiteColor];
        customStyle.doneBorderStyle = TFYBorderStyleFill;
    }
    return customStyle;
}
...
+ (instancetype)pickerStyleWithDoneTextColor:(UIColor *)doneTextColor {
    ...
    if (doneTextColor) {
        ...
        customStyle.doneBtnFrame = CGRectMake(TFYGetKeyWindow().bounds.size.width - 60, 4, 60, 40);
        customStyle.doneBtnTitle = [NSBundle picker_localizedStringForKey:@"完成" language:customStyle.language];
...
+ (instancetype)pickerStyleWithDoneBtnImage:(UIImage *)doneBtnImage {
    ...
    if (doneBtnImage) {
        ...
        customStyle.doneBtnFrame = CGRectMake(TFYGetKeyWindow().bounds.size.width - 44, 4, 40, 40);
    }
    return customStyle;
}
...
+ (void)picker_setView:(UIView *)view roundingCorners:(UIRectCorner)corners withRadius:(CGFloat)radius {
