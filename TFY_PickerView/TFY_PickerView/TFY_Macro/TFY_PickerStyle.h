...
// 边框样式（左边取消按钮/右边确定按钮）
typedef NS_ENUM(NSInteger, TFYBorderStyle) {
    /** 无边框（默认） */
    TFYBorderStyleNone = 0,
    /** 有圆角和边框 */
    TFYBorderStyleSolid,
    /** 仅有圆角 */
    TFYBorderStyleFill
...
@interface TFY_PickerStyle : NSObject
...
/** 设置 cancelBtn 的边框样式 */
@property (nonatomic, assign) TFYBorderStyle cancelBorderStyle;
...
/** 设置 doneBtn 的边框样式 */
@property (nonatomic, assign) TFYBorderStyle doneBorderStyle;
...
/////// 日期选择器单位样式（showUnitType == TFY_ShowUnitTypeOnlyCenter 时生效。暂不支持日期选择器前4种类型 ）///////
