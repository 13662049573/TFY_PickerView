...
+ (void)showPickerWithTitle:(NSString *)title
              dataSourceArr:(NSArray *)dataSourceArr
                ...
                resultBlock:(TFYStringResultModelBlock)resultBlock {
...
+ (void)showPickerWithTitle:(NSString *)title
              dataSourceArr:(NSArray *)dataSourceArr
               ...
               isAutoSelect:(BOOL)isAutoSelect
                resultBlock:(TFYStringResultModelBlock)resultBlock {
    // 创建选择器
    TFY_StringPickerView *strPickerView = [[TFY_StringPickerView alloc]init];
    strPickerView.pickerMode = TFYStringPickerComponentSingle;
...
+ (void)showMultiPickerWithTitle:(NSString *)title
                   dataSourceArr:(NSArray *)dataSourceArr
                    selectIndexs:(NSArray <NSNumber *>*)selectIndexs
                     resultBlock:(TFYStringResultModelArrayBlock)resultBlock {
...
+ (void)showMultiPickerWithTitle:(NSString *)title
    ...
    // 创建选择器
    TFY_StringPickerView *strPickerView = [[TFY_StringPickerView alloc]init];
    strPickerView.pickerMode = TFYStringPickerComponentMulti;
    strPickerView.title = title;
    strPickerView.dataSourceArr = dataSourceArr;
...
+ (void)showLinkagePickerWithTitle:(nullable NSString *)title
    ...
    // 创建选择器
    TFY_StringPickerView *strPickerView = [[TFY_StringPickerView alloc]init];
    strPickerView.pickerMode = TFYStringPickerComponentLinkage;
    strPickerView.title = title;
    strPickerView.dataSourceArr = dataSourceArr;
    strPickerView.selectIndexs = selectIndexs;
    strPickerView.isAutoSelect = isAutoSelect;
...
- (void)handlerPickerData {
    ...
    if (self.pickerMode == TFYStringPickerComponentSingle) {
        _dataSourceException = [item isKindOfClass:[NSArray class]];
    } else if (self.pickerMode == TFYStringPickerComponentMulti) {
        _dataSourceException = [item isKindOfClass:[NSString class]];
    } else if (self.pickerMode == TFYStringPickerComponentLinkage) {
        _dataSourceException = ![item isKindOfClass:[TFY_ResultModel class]];
    }
    ...
    if (self.pickerMode == TFYStringPickerComponentSingle) {
        ...
        if (self.selectIndex > 0 && self.selectIndex < self.mDataSourceArr.count) {
            ...
            if (self.mSelectValue) {
                ...
                if ([item isKindOfClass:[TFY_ResultModel class]]) {
                    for (NSInteger i = 0; i < self.mDataSourceArr.count; i++) {
                        TFY_ResultModel *model = self.mDataSourceArr[i];
                ...
                if (self.mSelectValues.count > 0 && i < self.mSelectValues.count) {
                    ...
                    if ([item isKindOfClass:[TFY_ResultModel class]]) {
                        for (NSInteger j = 0; j < itemArr.count; j++) {
                            TFY_ResultModel *model = itemArr[j];
        ...
        TFY_ResultModel *selectModel = nil;
        BOOL hasNext = YES;
        NSInteger i = 0;
...
- (NSArray <TFY_ResultModel *>*)getNextDataArr:(NSArray *)dataArr selectModel:(TFY_ResultModel *)selectModel {
    NSMutableArray *tempArr = [[NSMutableArray alloc]init];
    // parentKey = @"-1"，表示是第一列数据
    NSString *key = selectModel ? selectModel.key : @"-1";
    for (TFY_ResultModel *model in dataArr) {
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView {
    switch (self.pickerMode) {
        case TFYStringPickerComponentSingle:
            return 1;
            break;
        case TFYStringPickerComponentMulti:
        case TFYStringPickerComponentLinkage:
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view {
    ...
    if (self.pickerMode == TFYStringPickerComponentSingle) {
        ...
        if ([item isKindOfClass:[TFY_ResultModel class]]) {
            TFY_ResultModel *model = (TFY_ResultModel *)item;
            label.text = model.value;
        } else {
            label.text = item;
        }
    } else if (self.pickerMode == TFYStringPickerComponentMulti || self.pickerMode == TFYStringPickerComponentLinkage) {
        NSArray *itemArr = self.mDataSourceArr[component];
...
- (TFY_ResultModel *)getResultModel {
    ...
    if ([item isKindOfClass:[TFY_ResultModel class]]) {
        ...
        TFY_ResultModel *model = [[TFY_ResultModel alloc]init];
        model.index = self.selectIndex;
        model.value = item;
        return model;
    }
...
- (void)addPickerToView:(UIView *)view {
    ...
    self.doneBlock = ^{
        // 点击确定按钮后，执行block回调
        if (weakSelf.pickerMode == TFYStringPickerComponentSingle) {
            if (weakSelf.resultModelBlock) {
                weakSelf.resultModelBlock([weakSelf getResultModel]);
            }
        } else if (weakSelf.pickerMode == TFYStringPickerComponentMulti || weakSelf.pickerMode == TFYStringPickerComponentLinkage) {
